...
ensureColumn("sessions", "completion_success", "INTEGER DEFAULT 0");
...
function seedOtherUsersSessionsForTesting() {
  ...
  const demo = [
    ...
    { topic: "Physics drills", privacy: "public", h: 0, m: 45, admin: admins[0] },
    { topic: "Chem revision", privacy: "public", h: 2, m: 0, admin: admins[0] },
    { topic: "AI Study Group", privacy: "public", h: 1, m: 30, admin: admins[0] },
    { topic: "Exam Practice", privacy: "public", h: 0, m: 50, admin: admins[0] },
  ]

  const demoTopics = demo.map((s) => s.topic)
  const placeholders = demoTopics.map(() => "?").join(", ")
  db.prepare(`
...
// API: list successfully completed sessions for the garden
app.get("/api/sessions/completed", (_req, res) => {
  try {
    const rows = db.prepare(
      `SELECT s.id, s.privacy, s.topic, s.duration_hours, s.duration_minutes,
              s.start_time, s.ended_at, s.completion_success,
              s.invited_ids, s.todos, s.personal_todos, s.ai_todos, s.ai_generated,
              u.username AS admin_username
       FROM sessions s
       JOIN users u ON s.admin_user_id = u.id
       WHERE s.ended_at IS NOT NULL
         AND s.completion_success = 1
    ...
    const data = rows.map(r => ({
      ...
      start_time: r.start_time,
      ended_at: r.ended_at,
      completion_success: Number(r.completion_success) === 1,
      invited_ids: r.invited_ids ? JSON.parse(r.invited_ids) : [],
      todos: r.todos ? JSON.parse(r.todos) : [],
      personal_todos: r.personal_todos ? JSON.parse(r.personal_todos) : [],
      ai_todos: r.ai_todos ? JSON.parse(r.ai_todos) : [],
...
// Demo-only: private session invitations for presentations
app.get("/api/demo/invitations", (_req, res) => {
  try {
    const realUserId = getRealUserId()
    const admin = db
      .prepare("SELECT id, username FROM users WHERE is_real = 0 ORDER BY id LIMIT 1")
      .get()

    if (!admin) return res.json({ data: [] })

    const now = Date.now()
    const topic = "Private Exam Sprint"
    const tasks = [
      { id: "demo-task-1", text: "Review lecture summary", done: false },
      { id: "demo-task-2", text: "Solve two practice questions", done: false },
      { id: "demo-task-3", text: "Prepare final notes", done: false },
    ]
    const invitedIds = [realUserId, 2, 3].filter(Boolean)

    db.prepare("DELETE FROM sessions WHERE topic = ?").run(topic)
...
app.post("/api/sessions/:id/end", (req, res) => {
  ...
  try {
    ...
    db.prepare("UPDATE sessions SET ended_at = ?, completion_success = ? WHERE id = ?").run(Date.now(), success, id)
